...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr 12 10:07:50 2021
...
def get_H_G (path_matrice):   
    ##Reading the matrices
    matrixText = open(path_matrice,"r")
...
def decode_GALA (H,x):
    ...
    #Step 3 (Majority voting)
        ...
        l+=1
        xarr.append(xDecoded)
    return xDecoded,xarr
########################DEBUT CODE###############################
#On a 3 types de buffers : Bytes, bin, et empty qui contient les 3 pixels
root=tk.Tk()
root.withdraw()
print("Quelle image voulez-vous?")
path_image=filedialog.askopenfilename()
print("Ou stocker les resultats?")
...
data_bad=[]
H,G=get_H_G(path_matrice)

assert np.count_nonzero((G@H.transpose())%2)==0 and H.size>2,"Veuillez reverifier vos matrices"
...
pilImage.save(path_results+'/video_b.png') #Sauvegarder l'image
##Image corr
...
for i in range(len(data_bad)):
    if(not np.array_equal(data_bad[i],data_good[i])):
        buf,buf_it=decode_GALA(H,data_bad[i])
    else:
        buf=data_bad[i]
        buf_it=[data_bad[i],data_bad[i],data_bad[i],data_bad[i],data_bad[i]]
    if(i%1000==0):
        print(i)
    #Code qui vient d'etre ajoute
    data_bad_c.append(buf) #Pour compter le BER apres correction
    data_bad_it.append(buf_it)
...
pilImagec.save(path_results+'/video_c.png') #Sauvegarder l'image    
...
print('--------------------Afficher iterations--------------------------')

for cnt in range(5):
    data_line_c=[]
    print(cnt)
    for i in range(len(data_bad_it)):
        buf_rec=np.concatenate((data_bad_it[i][cnt][0:35:1],data_bad_it[i][cnt][36:56:1]))
        buf_rec=np.append(buf_rec,data_bad_it[i][cnt][72])
        buf_rec=word_to_bytes(buf_rec)
        buf_rec=[x for x in buf_rec]#Bytes en tableau de int
        buf_rec.pop(0)#On enleve la synch
        #Fin du decodage pour mot initial
    ...
    matrice_recue_c=matrice_recue_c.astype('uint8')
    pilImagec = Image.fromarray(matrice_recue_c,'RGB')
    pilImagec.save(path_results+'/it'+str(cnt)+'.png') #Sauvegarder l'image  
    
#----------------------Writing in text file--------------------------------#
of=open(path_results+"/stats.txt","a+")#Open in append mode and creates new file if needed
of.write("#=================Resultats====================#\n")
of.write("BER avant correction : {}\n".format(BER_b))
of.write("BER apres correction : {}\n".format(BER_c))
of.write("Rapport de pixels errones : {}\n".format(FER))
of.close()
